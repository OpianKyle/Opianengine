...
/**
 * Get customer details including authorizations
 */
export const getCustomersByEmail = async (email: string): Promise<any[]> => {
  try {
    const response = await createPaystackRequest(`customer?email=${encodeURIComponent(email)}`);
    const responseData = await response.json();
    
    if (!response.ok || !responseData.status) {
      throw new Error(`Failed to get customer details: ${responseData.message}`);
    }
    
    return responseData.data || [];
  } catch (error) {
    console.error('Error fetching Paystack customer details:', error);
    throw error;
  }
};

/**
 * Check if a customer has authorizations (saved payment methods)
 * This is required for creating subscriptions
 */
export const hasCustomerAuthorizations = async (email: string): Promise<boolean> => {
  try {
    // First get customer by email to get customer code
    ...
    // Get the customer code from the first match
    const customerCode = customers[0].customer_code;
    
    // Now get the customer details with the code to get authorizations
    const response = await createPaystackRequest(`customer/${customerCode}`);
    const responseData = await response.json();
    
    if (!response.ok || !responseData.status) {
      console.log(`Could not fetch details for customer code: ${customerCode}`);
      return false;
    }
    
    // Check if customer has authorizations
    const authorizations = responseData.data?.authorizations || [];
    const hasValidAuth = authorizations.some((auth: any) => auth.reusable === true);
    
    console.log(`Customer ${email} has ${authorizations.length} authorizations, valid: ${hasValidAuth}`);
    return hasValidAuth;
  } catch (error) {
    console.error('Error checking customer authorizations:', error);
    return false; 
  }
};