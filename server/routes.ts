...
// SMTP Test route - test connection to SMTP server without sending an email
  ...
  // Customer redemptions endpoint
  // Cash Deposits endpoint for customer cash wallet
  app.get("/api/customer/cash-deposits", async (req, res) => {
    if (!req.user) {
      return res.status(401).json({ error: "Unauthorized" });
    }
    ...
    try {
      ...
      const depositsWithCashValue = deposits.map((deposit) => ({
        ...deposit,
  ...
  app.get("/api/customer/redemptions", async (req, res) => {
    if (!req.user) {
      return res.status(401).json({ error: "Unauthorized" });
    }
    ...
    try {
      ...
      console.log('Found redemptions for user:', {
        userId,
        count: redemptions && Array.isArray(redemptions) ? redemptions.length : 0
      });
      
      // Format the redemptions for the frontend
      const formattedRedemptions = redemptions && Array.isArray(redemptions) ? 
        redemptions.map((r: any) => ({
          id: r.id,
          points: Math.abs(r.points || 0), // Convert to positive for display
          cashAmount: r.cash_amount || (Math.abs(r.points || 0) * 0.015).toFixed(2),
  ...
  app.post("/api/admin/cash-redemptions/:id/process", async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ 
        error: "Failed to process cash redemption",
        message: error.message
      });
    } finally {
      await connection.end();
