...
/**
 * Endpoint for admin to assign cash deposits to customers
 * Converts a specified amount of points to cash and adds to customer's cash balance
 */
router.post('/customer/:id/assign-cash-deposit', checkAdmin, async (req: any, res) => {
  const connection = await pool.getConnection();
  ...
  try {
    ...
    if (!customerId || !points || points <= 0) {
      return res.status(400).json({ error: 'Invalid parameters. customerId and points (> 0) are required.' });
    }
    
    // Begin transaction
    await connection.beginTransaction();
    
    // 1. Check if customer exists and has enough points
    ...
    // 2. Calculate cash value (R0.015 per point)
    const cashAmount = Number((points * 0.015).toFixed(2));
    const customDescription = description || `Admin assigned cash deposit - converted ${points} points to R${cashAmount}`;
    ...
    // 4. Add cash to customer's cash_balance
    await connection.query(
      `UPDATE users SET cash_balance = COALESCE(cash_balance, 0) + ? WHERE id = ?`,
      [cashAmount, customerId]
    );
    
    // 5. Record points transaction
    await connection.query(
      `INSERT INTO transactions (user_id, points, description, type) VALUES (?, ?, ?, ?)`,
      [customerId, -points, customDescription, 'CASH_CONVERSION']
    );
    
    // 6. Record cash wallet transaction
    await connection.query(
      `INSERT INTO cash_wallet (user_id, amount, description) VALUES (?, ?, ?)`,
      [customerId, cashAmount, customDescription]
    );
    
    // Log admin action
    await logAdminAction({
      adminId: req.user.id,
      actionType: 'ASSIGN_CASH_DEPOSIT' as any,
      details: `Admin ${req.user.email} assigned R${cashAmount} cash deposit to customer ${customer.email} by converting ${points} points`,
      userId: customerId
    });
    ...
    res.json({ 
      success: true, 
      message: 'Cash deposit assigned successfully',
      data: {
        customerId,
    ...
    await connection.rollback();
    console.error('Error assigning cash deposit:', error);
    res.status(500).json({ error: 'Failed to assign cash deposit' });
  } finally {
    connection.release();
  }
});
