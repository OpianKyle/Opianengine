...
import { generateReference, initializeTransaction, verifyTransaction } from '../utils/paystack';
...
router.post('/initialize', async (req, res) => {
  try {
    const { amount, purpose, planCode, metadata = {} } = req.body;
    ...
    const { createOrGetCustomer } = await import('../utils/paystack');
          ...
          try {
            ...
            if (customerCode && planCode) {
              try {
                ...
                // Update our database with the Paystack subscription code and email token
                await connection.query(
                  `UPDATE subscriptions SET 
                  ...
                  [
                    ...
                    customerCode,
                    subscriptionData.email_token || null,
                    new Date(),
                    transaction.metadata.subscription_id
                  ]
                );
                
                console.log(`Updated subscription with Paystack code ${subscriptionData.subscription_code} and email token`);
...
// Handle GET request for callback to support both types
router.get('/callback', async (req, res) => {
  try {
    const reference = req.query.reference || req.query.trxref;
    console.log('Payment callback received (GET):', { reference, query: req.query });
    
    // Create the same redirect logic as the POST handler
    // Check if the user is authenticated
    if (req.isAuthenticated()) {
      const redirectUrl = `/subscription?paymentComplete=true&reference=${reference}`;
      console.log('User authenticated, redirecting to subscription page:', redirectUrl);
      return res.redirect(redirectUrl);
    } else {
      // Redirect to login page with payment information for unauthenticated users
      const redirectUrl = `/login?paymentComplete=true&reference=${reference}`;
      console.log('User not authenticated, redirecting to login page:', redirectUrl);
      return res.redirect(redirectUrl);
    }
  } catch (error) {
  ...
  try {
    ...
    try {
      ...
      if (transaction.metadata) {
        ...
        // Try to create subscription in Paystack if needed
        if (customerCode && planCode) {
          try {
            const { createSubscription } = await import('../utils/paystack');
            const subscriptionData = await createSubscription(
            ...
            // Update database with Paystack subscription details
            await connection.query(
              `UPDATE subscriptions SET 
               paystack_subscription_code = ?, 
               paystack_customer_code = ?,
            ...
            console.log(`Created Paystack subscription ${subscriptionData.subscription_code} for local subscription ${subscriptionId}`);
          } catch (subscriptionError) {
            console.error('Failed to create Paystack subscription:', subscriptionError);
          }
        }
      }
...
async function handleSuccessfulCharge(data) {
  try {
    ...
    console.log('Successful charge webhook:', { 
      reference, 
      amount: data.amount, 
      metadata 
    });
